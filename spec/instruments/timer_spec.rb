...
describe Metrics::Instruments::Timer do
  ...
  it "should accept options for the constructor" do
    @timer = Metrics::Instruments::Timer.new({:interval => "10 seconds", :rateunit => "5 seconds"})
  end
  
  it "should tick properly" do
    @timer = Metrics::Instruments::Timer.new({:interval => "5 seconds", :rateunit => "1 second", :nothread => true})
    @timer.mark(100)
    @timer.tick()
